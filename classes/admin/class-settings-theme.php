...
class Settings_Theme {
	 ...
	 * @since 1.0.0
	 *
	 * @var      object \lsx\testimonials\classes\admin\Settings_Theme()
	 */
	protected static $instance = null;

	/**
	 * Will return true if this is the LSX testimonials settings page.
	 *
	 * @var array
	 */
	 ...
	 * @since 1.0.0
	 *
	 * @return    object \lsx\testimonials\classes\admin\Settings_Theme()    A single instance of this class.
	 */
	...
	public function disable_cmb2_styles( $enabled ) {
		if ( is_admin() ) {
			...
			if ( is_object( $current_screen ) && 'testimonial_page_lsx_testimonials_options' === $current_screen->id ) {
				$enabled = false;
			}
		}
		return $enabled;
	...
	public function generate_navigation( $cmb_id, $object_id, $object_type, $cmb2_obj ) {
		if ( 'lsx_testimonials_settings' === $cmb_id && 'lsx_testimonials_options' === $object_id && 'options-page' === $object_type ) {
	...
	public function navigation_js( $cmb_id, $object_id, $object_type, $cmb2_obj ) {
		if ( 'lsx_testimonials_settings' === $cmb_id && 'lsx_testimonials_options' === $object_id && 'options-page' === $object_type ) {
			?>
			<script>
				var LSX_TESTIMONIALS_CMB2 = Object.create( null );

				;( function( $, window, document, undefined ) {

					'use strict';

					LSX_TESTIMONIALS_CMB2.document = $(document);

					/**
					 * Start the JS Class
					 */
					LSX_TESTIMONIALS_CMB2.init = function() {
						var tab = LSX_TESTIMONIALS_CMB2.urlParam( 'cmb_tab' );
						if ( 0 === tab || '0' === tab ) {
							tab = '';
						}
						LSX_TESTIMONIALS_CMB2.addTabInput( tab );
						LSX_TESTIMONIALS_CMB2.prepNavigation( tab );
						LSX_TESTIMONIALS_CMB2.watchNavigation();
					};

					LSX_TESTIMONIALS_CMB2.addTabInput = function( tab = '' ) {
						var counter = 1;
						$( "form.cmb-form" ).append('<input type="hidden" name="cmb_tab" value="' + tab + '" />');
					}

					LSX_TESTIMONIALS_CMB2.prepNavigation = function( tab = '' ) {
						var counter = 1;
					...
					LSX_TESTIMONIALS_CMB2.watchNavigation = function() {
					...
					LSX_TESTIMONIALS_CMB2.urlParam = function(name){
					...
					LSX_TESTIMONIALS_CMB2.document.ready( function() {
						LSX_TESTIMONIALS_CMB2.init();
					} );
